...
err_code graph_api::wait(unsigned long long cycleTimeout)
{
    ...
    if (cycleTimeout > 0xFFFFFFFFFFFF)
        return errorMsg(err_code::user_error, "ERROR: abr::graph::wait: Max cycle timeout value can be 0xFFFFFFFFFFFF.");
...
err_code checkRTPConfigForUpdate(const rtp_config* pRTPConfig, const graph_config* pGraphConfig, size_t numBytes, bool isRunning)
{
    ...
    {
        if (pRTPConfig->isPL)
            return errorMsg(err_code::user_error, "ERROR: abr::graph::update to connected RL input RTP is not supported.");
        else //AIE RTP
        {
            ...
            {
                if (isRunning)
                    return errorMsg(err_code::user_error, "ERROR: abr::graph::update to connected asynchronous input RTP is not allowed during graph run.");
            }
            // Does not support connected sync input RTP
            else
                return errorMsg(err_code::user_error, "ERROR: abr::graph::update to connected synchronous input RTP is not supported.");
        }
    }
...
err_code graph_api::update(const rtp_config* pRTPConfig, const void* pValue, size_t numBytes)
{
    ...
    if (driverStatus != AieRC::XAIE_OK)
        return errorMsg(err_code::aie_driver_error, "ERROR: abr::graph::update: XAieTile_LockAcquire timeout or AIE driver error.");
...
err_code graph_api::read(const rtp_config* pRTPConfig, void* pValue, size_t numBytes)
{
    ...
    if (driverStatus != AieRC::XAIE_OK)
        return errorMsg(err_code::aie_driver_error, "ERROR: abr::graph::read: XAieTile_LockAcquire timeout or AIE driver error.");
...
err_code gmio_api::configure()
{
    ...
    if (pGMIOConfig->type == gmio_config::gm2aie || pGMIOConfig->type == gmio_config::aie2gm)
    {
        ...
        return errorMsg(err_code::aie_driver_error, "ERROR: abr::gmio_api::configure: GM - PL connection is not supported in GMIO AIE API.");
...
err_code dma_api::configureBdWaitQueueEnqueueTask(int tileType, uint8_t column, uint8_t row, int dir, uint8_t channel, uint32_t repeatCount, bool enableTaskCompleteToken, std::vector<uint8_t> bdIds, std::vector<dma_api::buffer_descriptor> bdParams)
{
    ...
    if (config_manager::s_pDevInst->DevProp.DevGen == XAIE_DEV_GEN_AIE)
        return errorMsg(err_code::internal_error, "ERROR: abr::dma_api::enqueueTask: Does not support AIE architecture.");
    ...
    if (bdIds.size() != bdParams.size())
        return errorMsg(err_code::internal_error, "ERROR: abr::dma_api::enqueueTask: The number of BD IDs and the number of BDs are different.");
...
err_code lock_api::acquireLock(int tileType, uint8_t column, uint8_t row, unsigned short lockId, int8_t acqVal)
{
    ...
    if (driverStatus != AieRC::XAIE_OK)
        return errorMsg(err_code::aie_driver_error, "ERROR: abr::lock_api::acquireLock: XAieTile_LockAcquire timeout or AIE driver error.");
    else
        return err_code::ok;
}
